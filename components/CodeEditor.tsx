...
import { FileCode2, FileJson, FileType, FileText, Braces, Palette } from 'lucide-react';
import { ReactIcon, TypeScriptIcon, JavaScriptIcon, NextjsIcon, PythonIcon } from './Icons';
...
const CodeEditor: React.FC<CodeEditorProps> = ({ title, code, onChange, language, readOnly = false, highlight = false }) => {
  ...
  const getHeaderIcon = () => {
      ...
      if (name.includes('json')) return <Braces className="w-4 h-4 text-yellow-500" />;
      if (name.includes('css')) return <Palette className="w-4 h-4 text-pink-400" />;
      if (name.includes('md')) return <FileText className="w-4 h-4 text-gray-400" />;
      if (name.includes('tsx') || name.includes('jsx')) return <ReactIcon className="w-4 h-4 text-blue-400" />;
      if (name.endsWith('.ts')) return <TypeScriptIcon className="w-4 h-4 text-blue-500" />;
      if (name.endsWith('.js')) return <JavaScriptIcon className="w-4 h-4 text-yellow-400" />;
      if (name.endsWith('.py')) return <PythonIcon className="w-4 h-4 text-blue-300" />;
  ...
  return (
    <div className={`flex flex-col h-full rounded-xl overflow-hidden border transition-all duration-300 ${highlight ? 'border-brand-500/50 shadow-[0_0_20px_rgba(34,197,94,0.1)]' : 'border-dark-700'}`}>
      <div className="bg-dark-900 px-4 py-2 border-b border-dark-700 flex justify-between items-center shrink-0">
        ...
        <span className="text-xs px-2 py-0.5 rounded bg-dark-700 text-gray-400 font-mono uppercase">{language}</span>
      </div>
