...
import { 
    CheckCircle2, X, Download, ShieldCheck, Zap, TestTube, 
    ArrowRight, Activity, Cpu, Layers, Award, FileCode2
} from 'lucide-react';
import { ReactIcon, NextjsIcon, TypeScriptIcon } from './Icons';
...
const MigrationReportModal: React.FC<MigrationReportModalProps> = ({ report, onClose, onDownload }) => {
    return (
            ...
            <div className="bg-dark-900 border border-dark-700 rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl flex flex-col relative">
                ...
                <div className="p-6 border-b border-dark-700 flex items-start justify-between bg-gradient-to-r from-dark-900 to-dark-800">
                    <div className="flex gap-4">
                        ...
                        <div>
                            <h2 className="text-2xl font-bold text-white tracking-tight">Migration Accomplished</h2>
                    ...
                    <button onClick={onClose} className="p-2 hover:bg-dark-700 rounded-lg transition-colors text-gray-400 hover:text-white">
                        <X className="w-6 h-6" />
                    </button>
                </div>
                ...
                <div className="p-6 space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="bg-dark-800 rounded-xl p-5 border border-dark-700 flex flex-col gap-3 relative overflow-hidden group">
                            ...
                            <div className="flex items-center justify-between">
                                <span className="text-sm font-semibold text-gray-400 uppercase tracking-wider">Modernization Score</span>
                                <span className={`text-xs font-bold px-2 py-0.5 rounded ${report.modernizationScore > 80 ? 'bg-brand-500/20 text-brand-400' : 'bg-yellow-500/20 text-yellow-400'}`}>
                                    {report.modernizationScore > 80 ? 'EXCELLENT' : 'GOOD'}
                                </span>
                            </div>
                            <div className="flex items-end gap-2">
                                <span className="text-4xl font-bold text-white">{report.modernizationScore}</span>
                            ...
                            <div className="w-full bg-dark-900 h-1.5 rounded-full mt-2 overflow-hidden">
                                <div className="h-full bg-brand-500 rounded-full transition-all duration-1000" style={{ width: `${report.modernizationScore}%` }} />
                            </div>
                        </div>
                        ...
                        <div className="bg-dark-800 rounded-xl p-5 border border-dark-700 flex flex-col gap-3 relative overflow-hidden">
                            ...
                            <span className="text-sm font-semibold text-gray-400 uppercase tracking-wider">Type Safety Coverage</span>
                            <div className="flex items-end gap-2">
                                <span className="text-4xl font-bold text-blue-400">{report.typeScriptCoverage}%</span>
                            </div>
                            <div className="text-xs text-gray-500 mt-auto">
                                Converted to strict TypeScript
                            </div>
                             <div className="w-full bg-dark-900 h-1.5 rounded-full mt-2 overflow-hidden">
                                <div className="h-full bg-blue-500 rounded-full transition-all duration-1000" style={{ width: `${report.typeScriptCoverage}%` }} />
                            </div>
                        </div>
                        ...
                        <div className="bg-dark-800 rounded-xl p-5 border border-dark-700 flex flex-col gap-3 relative overflow-hidden">
                            ...
                            <span className="text-sm font-semibold text-gray-400 uppercase tracking-wider">Test Suite</span>
                            <div className="flex items-end gap-2">
                                <span className="text-4xl font-bold text-purple-400">{report.testsGenerated}</span>
                            ...
                            <div className="text-xs text-gray-500 mt-auto">
                             ...
                             <div className="w-full bg-dark-900 h-1.5 rounded-full mt-2 overflow-hidden">
                                <div className="h-full bg-purple-500 rounded-full transition-all duration-1000" style={{ width: `${report.testCoverage}%` }} />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        ...
                        <div className="bg-dark-800/50 rounded-xl border border-dark-700 p-5">
                             <h3 className="text-sm font-bold text-gray-300 mb-4 uppercase tracking-wide">Project Statistics</h3>
                             <div className="space-y-4">
                                <div className="flex justify-between items-center border-b border-dark-700 pb-2">
                                    <span className="text-gray-400 text-sm">Total Files Processed</span>
                                    <span className="text-white font-mono">{report.totalFiles}</span>
                                </div>
                                <div className="flex justify-between items-center border-b border-dark-700 pb-2">
                                    <span className="text-gray-400 text-sm">New Dependencies Added</span>
                                    <span className="text-white font-mono">{report.newDependencies}</span>
                                </div>
                                <div className="flex justify-between items-center border-b border-dark-700 pb-2">
                                    <span className="text-gray-400 text-sm">Architecture Type</span>
                                    <span className="text-brand-400 font-mono text-sm">App Router (Server Components)</span>
                                </div>
                             </div>
                        </div>
                    </div>
