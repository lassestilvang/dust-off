...
const getFileIconConfig = (filename: string) => {
  ...
  if (name === 'tsconfig.json')
    return { icon: TypeScriptIcon, color: 'text-blue-500' };
  ...
  if (name.endsWith('.jsx'))
    return { icon: ReactIcon, color: 'text-yellow-400' };
  if (name.endsWith('.js'))
    return { icon: JavaScriptIcon, color: 'text-yellow-400' };
  ...
  if (name.match(/\.(png|jpg|jpeg|svg|ico|gif)$/))
    ...
    return (
      <div>
        ...
        >
          ...
          <span className="text-xs font-medium truncate group-hover:text-gray-300 transition-colors">
            {node.name}
          </span>
        </div>
  ...
  return (
    <div className="h-full flex flex-col bg-dark-900 rounded-xl border border-dark-700 overflow-hidden shadow-inner">
      <div className="px-2 py-2 border-b border-dark-700 bg-dark-900 flex gap-1">
        ...
        >
          <Database
            className={`w-3.5 h-3.5 ${activeTree === 'source' ? 'text-blue-400' : 'text-gray-500'}`}
          />
          Legacy
        </button>
        <button
